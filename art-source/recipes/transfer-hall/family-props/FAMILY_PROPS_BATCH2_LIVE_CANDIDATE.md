...
Status: **`LIVE_CANDIDATE` — pending deployed PC/mobile Art-Director QA**.

This batch integrates four isolated strict-top-down family-area concepts explicitly approved for live testing: two plant types, a coffee machine and a hologram pedestal. The rejected carpet and earlier non-top-down concept-sheet props are not part of this batch.
...
The coffee machine must be approached/read from its lower side, therefore it is placed at the **upper wall** with its service/access face pointing into the room.
...
The 64×128 runtime source deliberately carries transparent space above the visible machine so the first visible pixels enter the room below the accepted upper-wall fascia without requiring another per-GID wall-placement hack.

### Planter trough

The approved horizontal source was deterministically rotated 90° for a vertical placement along the left side of the Family area.

```text
Prop layer:       FloorProps
Prop GIDs:        181–182
Placement:        col 2,row 7; 1×2
...
Shadow layer:     FloorFX
Shadow GIDs:      183–184
Shadow placement: col 2,row 7; 1×2
...
Shadow layer:     FloorFX
Shadow GID:       186
Shadow placement: col 5,row 7; 1×1
...
Shadow layer:     FloorFX
Shadow GID:       188
Shadow placement: col 4,row 8; 1×1
...
The batch adds deliberately smaller collision footprints than the visual foliage/effect envelopes:

```text
family-coffee-machine-solid   x 5.18,y 1.52,w 0.64,h 0.82
family-planter-trough-solid   x 2.18,y 7.55,w 0.64,h 0.90
family-round-plant-solid      x 5.20,y 7.28,w 0.60,h 0.55
family-hologram-solid         x 4.18,y 8.22,w 0.64,h 0.62
```
...
- coffee machine is visibly attached to the upper-wall area and clearly accessed from below;
- neither coffee art nor its shadow visually paints over the wall fascia;
- planter trough reads as a vertical left-side plant element rather than a rotated mistake;
- round plant and hologram improve density without cluttering navigation;
- all four shadows ground their props without becoming dark graphic blobs;
- top-down perspective remains coherent with the accepted Family Table and Memory Console;
- no new prop visually competes with the Transfer hero focus.

Do not mark this batch `LIVE_ACCEPTED` until deployed Art-Director QA passes.
