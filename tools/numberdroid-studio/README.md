...
**Checkpoints 2C, 3, and 4 are implemented candidates but remain not user-accepted.** Checkpoint 2C adds strict exact-slice V2 assets, owner-gated lifecycle, durable bounded asset proposals, schema v9, ordinal **Slice 1–64** labels, and a sanitized offline bundle. Checkpoint 3 adds room/hallway archetypes and immutable variants, exact-version layered placement, deterministic validation, owner-only lifecycle, schema v10, and portable schema-v2 room round trips. Checkpoint 4 adds human-composed bounded tasks, durable isolated branch heads, timeline and control, semantic review/conflicts, truthful policy disposition, atomic accepted-change merge, compensating revert, schema v11, and task-bound MCP/UI surfaces. Non-task MCP discovery stays at 19 tools/four templates; a task-bound host exposes 30 tools/five templates while finalization, authority, merge/revert, export, and publish remain human-only. The user authorized continuation while deferring acceptance, so one combined 2C + 3 + 4 gate remains. PR #135 remains open, draft, and unmerged. Merge, release, providers, Numberdroid/runtime/repository export, materialization, and publication remain blocked. See the [2C candidate record](docs/CHECKPOINT_2C_STATUS.md), [Checkpoint 3 candidate record](docs/CHECKPOINT_3_STATUS.md), [Checkpoint 4 candidate record](docs/CHECKPOINT_4_STATUS.md), [Checkpoint 4 contract](docs/CHECKPOINT_4_CONTRACT.md), and [roadmap](docs/ROADMAP.md).
...
| Area | Current status |
| --- | --- |
| Product contract | Checkpoints 1, 2A, and 2B user-accepted; 2C, 3, and 4 implemented candidates awaiting one combined user gate |
| Standalone boundary | Accepted package/dependency boundary; extraction remains a later packaging task |
| Human UI | Accepted source/cutter workflow plus candidate asset, room/hallway, and delegated-task workspaces with branch timeline, comparison, review, merge, and revert |
| Agent access | Schema v11 keeps 19 tools/four templates without a task and exposes 30 tools/five templates only for a matching task branch; lifecycle/authority/release controls remain human-only |
| Persistence | SQLite schema v11 WAL ledger, normalized assets/rooms/proposals/task branches/reviews/merges/reverts, SHA-256 CAS, and sanitized schema-v1/v2 project bundles |
...
Schema v11 also verifies staged/claimed/abandoned source-intake references, permanent source/atlas/asset lineage, agent attempts, job state/event invariants, immutable asset and room versions, exact room asset pins, proposal decisions/applications, task branch documents/revisions/timelines/reviews/merge/revert lineage, rebuilt heads, imported APPLIED history, and state-specific CAS references. Backup and bundle export first run the complete semantic/CAS integrity precondition; portable export additionally refuses every nonterminal task branch and never transfers task authority/history. Roomless bundles remain canonical schema v1; projects with room semantics use strict schema v2 and round-trip without authority, machine paths, queues, or credentials. Migrations 0007–0010 retain their pinned checksums; migration 0011 is pinned to `f6ed508f3098e6cdeb3dca2af0a9be7baca12c18fcd9d518f75f4f353242639d`.
