...
**Checkpoints 2C and 3 are implemented and published candidates but remain not user-accepted.** Checkpoint 2C adds strict exact-slice V2 assets, owner-gated lifecycle, durable bounded asset proposals, schema v9, ordinal **Slice 1–64** labels, and a sanitized offline bundle. Checkpoint 3 adds room/hallway archetypes and immutable variants, exact-version layered placement, visible connector/clearance and intent editing, deterministic validation, owner-only validation/finalization/fork, schema v10, exactly 19 MCP tools/four resource templates with set-dressing proposal as the only agent room mutation, and portable schema-v2 room round trips. On 2026-08-22 the user deferred the 2C walkthrough, authorized Checkpoint 3, and requested this combined 2C + 3 acceptance gate; implementation and automated evidence are not acceptance. PR #135 remains open, draft, and unmerged. Merge, release, providers, Numberdroid/runtime/repository export, materialization, and publication remain blocked. See the [2C candidate record](docs/CHECKPOINT_2C_STATUS.md), [Checkpoint 3 candidate record](docs/CHECKPOINT_3_STATUS.md), [Checkpoint 3 contract](docs/CHECKPOINT_3_CONTRACT.md), and [roadmap](docs/ROADMAP.md).
...
| Area | Current status |
| --- | --- |
| Product contract | Checkpoints 1, 2A, and 2B user-accepted; 2C and 3 implemented candidates awaiting one combined user gate |
| Standalone boundary | Accepted package/dependency boundary; extraction remains a later packaging task |
| Human UI | Accepted source/cutter workflow plus candidate asset library and room/hallway designer with canvas, palette, inspector, findings, lifecycle, and proposal review |
| Agent access | Schema v10 advertises exactly 19 tools/four templates; direct room editing, decision/apply, validation/finalization/fork remain owner-only |
| Persistence | SQLite schema v10 WAL ledger, normalized assets/rooms/proposals/import history, SHA-256 CAS, and sanitized schema-v1/v2 project bundles |
| Numberdroid export | Adapter boundary specified; production publishing deferred |

## Run the accepted 2A/2B slices and the 2C/3 candidates locally
...
Open `http://127.0.0.1:4317`, choose **Create / load demo**, then use **Sources** for intake/cutting and **Assets** for exact-slice V2 review. In **Rooms**, create an archetype and DRAFT room or hallway, edit the three-layer intent trace and edge connectors, select an exact-version asset, then activate a coordinate cell to place it. Existing placements and connectors are selectable on the canvas and in structured lists; live findings link back to their targets. Validation, warning disposition, finalization, fork, and proposal decision/apply remain explicit owner controls. The default workspace is `.numberdroid-studio/`: `studio.sqlite` is authoritative, `artifacts/` is the CAS, and the private MCP pairing listener is loopback-only. Set `NUMBERDROID_STUDIO_DATA` to select another workspace.
...
Schema v10 also verifies staged/claimed/abandoned source-intake references, permanent source/atlas/asset lineage, agent attempts, job state/event invariants, immutable asset and room versions, exact room asset pins, proposal decisions/applications, rebuilt heads, imported APPLIED history, and state-specific CAS references. Backup and bundle export first run the complete semantic/CAS integrity precondition. Roomless bundles remain canonical schema v1; projects with room semantics use strict schema v2 and round-trip without authority, machine paths, queues, or credentials. Migrations 0007–0009 retain their accepted/pinned checksums; migration 0010 is pinned to `99d12a3a7ee7572dd9386bd183fb847631ceab0490b0190e3ba5f1b339cfd40e`.
...
- [Checkpoint 3 candidate record](docs/CHECKPOINT_3_STATUS.md)
