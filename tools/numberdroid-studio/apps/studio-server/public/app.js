...
import { createRoomEditorDraft, roomEditorSavedMatches } from './room-editor-draft.js';
...
function setRoomMutationPending(pending) {
  ...
  if (pending) for (const control of elements['workspace-content'].querySelectorAll('[data-room-control="zoom"], [data-room-zoom-slider]')) control.disabled = false;
  for (const preview of elements['workspace-content'].querySelectorAll('[data-room-view="preview"]')) {
    preview.disabled = pending || roomMoveTools.hasPending() || Boolean(state.roomUi.pendingPlacementAdd);
    preview.title = preview.disabled ? 'Save or discard Room changes first. Preview shows only the confirmed saved Room.' : 'Show the exact saved Room.';
  }
}
...
function exactSelectedRoomPaletteAsset(snapshot = state.project?.snapshot) {
  if (!state.roomUi.selectedPaletteAssetId || !state.roomUi.selectedPaletteAssetPin) return null;
  const pin = state.roomUi.selectedPaletteAssetPin;
  return currentAssetLibrary(snapshot).assets.find((asset) => asset.assetId === pin.assetId
...
function clearRoomPaletteAsset() {
  state.roomUi.selectedPaletteAssetId = null;
  state.roomUi.selectedPaletteAssetPin = null;
}
...
function sourceIntakePanel() {
  ...
  if (sourceIntakeFormCache && sourceIntakeFormContext === context) {
    ...
    submit.textContent = state.sourceMutationPending ? 'Importing…' : 'Import source image';
  ...
  const title = document.createElement('h2'); title.textContent = stagedIntake ? 'Resume staged source image' : 'Import source image';
  ...
  submit.type = 'submit'; submit.textContent = state.sourceMutationPending ? 'Importing…' : 'Import source image';
  ...
  form.append(heading, fields, status, reasonedDisabledControl(
    submit,
    'Importing source images requires a local editable Studio session.',
  ));
...
function sourceTechnicalDetails(item) {
  const details = document.createElement('details'); details.className = 'source-technical-details';
  const summary = document.createElement('summary'); summary.textContent = 'Technical details'; details.append(summary);
  const properties = [
    ['Source ID', item.id], ['Artifact', item.artifactUri], ['Origin', item.provenance.origin || 'legacy'],
    ['Lifecycle', item.lifecycle?.state ?? 'LEGACY_REGISTERED'],
    ['Review', item.review?.disposition ?? 'LEGACY_UNREVIEWED'], ['Seed', item.provenance.seed],
    ['Bytes', item.byteSize], ['Provider', item.provenance.provider || item.provenance.generator],
...
function reasonedDisabledControl(control, reason) {
  if (!control.disabled || !reason) return control;
  control.title = reason;
  const wrapper = document.createElement('span'); wrapper.className = 'disabled-control-reason';
  wrapper.tabIndex = 0; wrapper.title = reason; wrapper.dataset.disabledReason = reason;
  wrapper.setAttribute('aria-label', `${control.textContent}. Unavailable: ${reason}`);
  wrapper.append(control); return wrapper;
}

function sourceStatusBadge(presentation) {
  const badge = document.createElement('span'); badge.className = 'source-status-badge';
...
function renderSourcesNavigation(reviewCounts) {
  ...
  if (state.sourcesUi.tab === 'images') {
    ...
    importButton.dataset.sourceImportToggle = ''; importButton.setAttribute('aria-expanded', String(state.sourcesUi.importOpen));
    importButton.textContent = state.sourcesUi.importOpen ? 'Close import' : 'Import source image'; header.append(importButton);
  }
  ...
  const searchLabel = document.createElement('label'); searchLabel.className = 'sources-filter sources-filter-search';
  const searchCopy = document.createElement('span'); searchCopy.textContent = 'Search';
  const search = document.createElement('input'); search.type = 'search'; search.dataset.sourcesSearch = '';
  search.placeholder = state.sourcesUi.tab === 'images' ? 'Names, source IDs and provenance' : 'Work, source and saved outputs';
  search.value = state.sourcesUi.search; searchLabel.append(searchCopy, search);
  ...
  controls.append(searchLabel, attentionLabel); fragment.append(controls); return fragment;
}

function renderSourceImageCard(item) {
  ...
  if (lifecycle === 'REVIEWED' && review === 'PENDING') {
    for (const [disposition, label] of [['APPROVED', 'Approve source'], ['REJECTED', 'Reject source']]) {
      ...
      button.dataset.sourceReviewDecision = disposition; button.dataset.sourceId = item.id;
      if (disposition === 'REJECTED') button.className = 'secondary'; button.disabled = state.sourceMutationPending || !state.agentAccessCsrf;
      actions.append(reasonedDisabledControl(button, state.sourceMutationPending
        ? 'Another source change is being saved.'
        : 'Source review decisions require a local editable Studio session.'));
    }
  } else if (['IMPORTED', 'GENERATED'].includes(lifecycle) && review === 'PENDING') {
    const propose = document.createElement('button'); propose.type = 'button'; propose.textContent = 'Submit for review';
    propose.className = 'secondary'; propose.dataset.sourceReviewPropose = ''; propose.dataset.sourceId = item.id;
    propose.disabled = state.sourceMutationPending || !state.agentAccessCsrf;
    actions.append(reasonedDisabledControl(propose, state.sourceMutationPending
      ? 'Another source change is being saved.'
  ...
  sourceCard.append(actions, sourceTechnicalDetails(item)); return sourceCard;
}

function renderSourceImages(items) {
  const fragment = document.createDocumentFragment();
  if (state.sourcesUi.importOpen) fragment.append(sourceIntakePanel());
  const staged = stagedSourceIntakes(); if (staged) fragment.append(staged);
  const visible = filterSourceImages(items, state.sourcesUi);
  const count = document.createElement('p'); count.className = 'sources-result-count'; count.setAttribute('aria-live', 'polite');
  count.textContent = `${visible.length} of ${items.length} source ${items.length === 1 ? 'image' : 'images'}`; fragment.append(count);
  if (!visible.length) {
    const narrowed = state.sourcesUi.search.trim() || state.sourcesUi.attention !== 'all';
    fragment.append(emptyState(narrowed ? 'No matching source images' : 'No source images yet', narrowed
      ? 'Clear the search or status filter to see the other originals.'
      : 'Import a PNG or WebP together with the available information about where it came from.'));
    return fragment;
  }
  const grid = document.createElement('div'); grid.className = 'card-grid source-grid';
  for (const item of visible) grid.append(renderSourceImageCard(item)); fragment.append(grid); return fragment;
}
...
function renderWorkbenchCard(entry) {
  ...
  const details = document.createElement('details'); details.className = 'source-technical-details';
  const detailSummary = document.createElement('summary'); detailSummary.textContent = 'Technical details';
  const list = document.createElement('dl'); list.className = 'property-list';
  for (const [label, value] of [['Atlas ID', atlas.id], ['Source ID', atlas.sourceId], ['Definition version', atlas.definitionVersion], ['Latest preview job', atlas.latestPreviewJobId]]) {
    if (value === undefined || value === null || value === '') continue;
    const term = document.createElement('dt'); term.textContent = label; const description = document.createElement('dd'); description.textContent = String(value); list.append(term, description);
  }
  details.append(detailSummary, list); article.append(details); return article;
}
...
function rerenderSourcesWorkspace({ focus = null, selectionStart = null, selectionEnd = null } = {}) {
  if (state.workspace !== 'sources' || !state.project) return;
  elements['workspace-content'].replaceChildren(renderSources(state.project.snapshot.sources));
  ...
  if (!focus) return;
  const control = elements['workspace-content'].querySelector(focus);
  control?.focus({ preventScroll: true });
  if (control instanceof HTMLInputElement && selectionStart !== null) {
...
const ROOM_EDITOR_TOOLS = Object.freeze([
  ...
  ['PAINT_ROOM', '■', 'Room floor', 'Click a cell to make it usable room floor. Save changes keeps all Room edits together.'],
  ['PAINT_VOID', '▧', 'Outside room', 'Click a cell to exclude it from the room. Save changes keeps all Room edits together.'],
  ['PAINT_BLOCKED', '⊠', 'Blocked in room', 'Click a cell to keep it inside the room but block passage. Save changes keeps all Room edits together.'],
...
function roomShapeDraft(variant) {
  variant = variant ? roomMoveTools.project(variant) : variant;
...
function roomSurfaceContext() {
  ...
  return {
    ...
    dirtyShape: false, draftGeneration: roomMoveTools.getState().sequence, pinsReady: roomPinnedAssetsReady(variant),
    readOnly: !state.agentAccessCsrf || state.uiMode === 'remote',
    otherPending: roomMoveTools.isLocked(),
  };
}
...
const roomSurfaceTools = createRoomSurfaceTools({
  ...
  stage(plan) {
    const previous = structuredClone(displayedRoomVariant().placements);
    const removed = new Set(plan.removals.map(item => typeof item === 'string' ? item : item.placementId));
    const result = roomMoveTools.update(room => { room.placements = [...room.placements.filter(item => !removed.has(item.placementId)), ...structuredClone(plan.additions)]; }, 'surface');
    if (result) roomSurfaceDraftUndo = { sequence: roomMoveTools.getState().sequence, placements: previous };
    return result;
  },
  stageUndo() {
    if (!roomSurfaceDraftUndo || roomSurfaceDraftUndo.sequence !== roomMoveTools.getState().sequence) return false;
    const previous = roomSurfaceDraftUndo; roomSurfaceDraftUndo = null;
    return roomMoveTools.update(room => { room.placements = structuredClone(previous.placements); }, 'surface');
  },
  busyChanged(pending) {
...
function renderRoomToolOptions(variant) {
  ...
  status.dataset.dirty = String(moveState.dirty); status.dataset.conflict = String(Boolean(state.roomUi.shapeConflict));
  const actions = document.createElement('div'); actions.className = 'room-tool-actions';
  const save = roomControl('Save changes', 'editor-save'); save.classList.remove('secondary'); save.disabled = !moveState.dirty || moveState.locked || variant.lifecycle !== 'DRAFT';
  save.title = save.disabled ? (moveState.locked ? 'Resolve the current save first.' : 'No unsaved Room changes.') : 'Check the complete Room and save all changes in one version.';
  const reset = roomControl('Discard changes', 'editor-discard');
  reset.disabled = !moveState.dirty || moveState.locked; reset.title = reset.disabled ? 'No discardable Room changes.' : 'Discard all unsaved Room edits and return to the saved Room.'; actions.append(save, reset);
  if (['uncertain', 'refresh'].includes(moveState.phase)) {
    ...
    retry.textContent = moveState.phase === 'uncertain' ? 'Retry same save' : 'Read saved Room again';
...
let roomPinnedAssetsRequest = { generation: 0, controller: null };
function cancelRoomPinnedAssets() {
  roomPinnedAssetsRequest.controller?.abort();
  roomPinnedAssetsRequest = { generation: roomPinnedAssetsRequest.generation + 1, controller: null };
  state.roomUi.pinnedAssets = { key: null, status: 'idle', assets: [] };
}
function cancelPinnedAssetsOnWorkspaceExit(nextWorkspace) {
  if (state.workspace === 'rooms' && nextWorkspace !== 'rooms' && state.roomUi.pinnedAssets.status === 'loading') cancelRoomPinnedAssets();
}
function selectedRoomPinnedContext() {
  const { variant } = currentRoomVariant();
  return state.project && variant ? roomPinnedAssetsContext(state.project.projectId, state.project.revision, variant) : null;
}
function roomPinnedAssetsReady(variant, snapshot = state.project?.snapshot) {
  return !variant || variant.placements.every((placement) => Boolean(roomAssetSpan(exactRoomAsset(placement, snapshot), placement.rotation)));
}
function ensureRoomPinnedAssets(variant, snapshot, { retry = false } = {}) {
  const context = roomPinnedAssetsContext(state.project.projectId, state.project.revision, variant);
  const key = roomPinnedAssetsKey(context);
  if (variant.placements.every((placement) => currentAssetLibrary(snapshot).assets.some((asset) => roomAssetPinKey(asset) === roomAssetPinKey(placement)))) {
    if (state.roomUi.pinnedAssets.key !== null) cancelRoomPinnedAssets();
    return;
  }
  if (!retry && state.roomUi.pinnedAssets.key === key
      && (state.roomUi.pinnedAssets.status !== 'loading' || roomPinnedAssetsRequest.controller)) return;
  cancelRoomPinnedAssets();
  const controller = new AbortController(); const generation = roomPinnedAssetsRequest.generation;
  roomPinnedAssetsRequest.controller = controller;
  state.roomUi.pinnedAssets = { key, status: 'loading', assets: [] };
  const placements = structuredClone(variant.placements);
  const timer = setTimeout(() => controller.abort(), 8_000);
  const owns = () => generation === roomPinnedAssetsRequest.generation && state.workspace === 'rooms'
    && state.roomUi.pinnedAssets.key === key && roomPinnedAssetsKey(selectedRoomPinnedContext() ?? {}) === key;
  void (async () => {
    try {
      const value = await api(roomPinnedAssetsPath(context), { signal: controller.signal });
      if (!owns()) return;
      if (controller.signal.aborted) throw new Error('Pinned Asset read expired.');
      const assets = normalizeRoomPinnedAssets(value, context, placements);
      state.roomUi.pinnedAssets = { key, status: 'ready', assets };
    } catch {
      if (!owns()) return;
      state.roomUi.pinnedAssets = { key, status: 'unavailable', assets: [] };
    } finally {
      clearTimeout(timer);
...
elements['workspace-content'].addEventListener('click', (event) => {
  if (!event.target.closest('[data-room-pinned-assets-retry]') || state.workspace !== 'rooms') return;
  const { variant } = currentRoomVariant(); if (!variant) return;
  ensureRoomPinnedAssets(variant, state.project.snapshot, { retry: true }); renderWorkspace({ preserveRoomDraft: true });
});

function renderRoomPinnedAssetsStatus(variant, snapshot) {
  const ready = roomPinnedAssetsReady(variant, snapshot);
  const section = document.createElement('section'); section.className = 'room-panel';
  section.dataset.roomPinnedAssetsState = ready ? 'ready' : state.roomUi.pinnedAssets.status === 'loading' ? 'loading' : 'unavailable';
  if (ready) { section.hidden = true; return section; }
  const copy = document.createElement('p'); copy.setAttribute('role', 'status');
  copy.textContent = section.dataset.roomPinnedAssetsState === 'loading'
    ? 'Loading the exact Asset versions saved in this Room. Room edits wait until their geometry is available.'
    : 'The exact saved Asset versions could not be loaded. No replacement geometry is shown. Retry or choose another Room.';
  section.append(copy);
  if (section.dataset.roomPinnedAssetsState === 'unavailable') {
    const retry = document.createElement('button'); retry.type = 'button'; retry.className = 'secondary';
    retry.dataset.roomPinnedAssetsRetry = ''; retry.textContent = 'Retry saved Assets'; section.append(retry);
  }
  return section;
}
function applyRoomPinnedAssetsLock(editor, variant, snapshot) {
  if (roomPinnedAssetsReady(variant, snapshot)) return;
  editor.dataset.roomPinnedAssetsBlocked = 'true';
  for (const control of editor.querySelectorAll('button, input, select, textarea')) {
    if (control.matches('[data-room-zoom-slider], [data-room-layer], [data-room-control="zoom"], [data-room-control="editor-panel"], [data-room-control="finding"]')) continue;
    control.disabled = true;
  }
}

function exactRoomAsset(placement, snapshot = state.project?.snapshot) {
  const current = currentAssetLibrary(snapshot).assets.find((asset) => (
  ...
  if (snapshot !== state.project?.snapshot) return null;
  const context = selectedRoomPinnedContext(); const cached = state.roomUi.pinnedAssets;
  if (!context || cached?.status !== 'ready' || cached.key !== roomPinnedAssetsKey(context)) return null;
  return cached.assets.find((asset) => roomAssetPinKey(asset) === roomAssetPinKey(placement)) ?? null;
}

function roomAssetSpan(asset, rotation = 0) {
  const span = asset?.metadata?.spanTiles;
  if (!Number.isSafeInteger(span?.width) || !Number.isSafeInteger(span?.height) || span.width < 1 || span.height < 1) return null;
...
function roomPlacementVisual(asset, rotation) {
  ...
  visual.dataset.rotation = String(rotation);
  visual.style.setProperty('--room-placement-visual-rotation', `${rotation}deg`);
  visual.style.setProperty('--room-placement-visual-width', `${(authored.width / rotated.width) * 100}%`);
  visual.style.setProperty('--room-placement-visual-height', `${(authored.height / rotated.height) * 100}%`);
  return visual;
}

function roomRectsIntersect(left, right) {
  return left.x < right.x + right.width && left.x + left.width > right.x
    && left.y < right.y + right.height && left.y + left.height > right.y;
}

function roomRotatedLocalRect(rect, span, rotation) {
...
function roomPlacementGhostModel({ variant, snapshot, asset, anchor, rotation, layer, ignorePlacementId = null }) {
  const blockers = [];
  const span = asset?.metadata?.spanTiles;
  if (!asset) blockers.push('The exact version-pinned asset is unavailable.');
  if (!span) blockers.push('The asset has no authored logical footprint.');
  if (asset?.metadata?.rotationPolicy === 'fixed' && rotation !== 0) blockers.push('This asset has a fixed orientation.');
  const footprint = roomAssetSpan(asset, rotation);
  if (!footprint || !roomPinnedAssetsReady(variant, snapshot)) return { anchor, rotation, footprint: null, asset, layer, allowed: false, message: 'Exact saved Asset geometry is unavailable. Wait for it to load or retry before editing this Room.' };
  const envelope = { x: anchor.x, y: anchor.y, width: footprint.width, height: footprint.height };
  const inBounds = anchor.x >= 0 && anchor.y >= 0
    && anchor.x + footprint.width <= variant.width && anchor.y + footprint.height <= variant.height;
  if (!inBounds) blockers.push('The rotated logical footprint exceeds the room bounds.');
  if (inBounds) {
    const cells = [];
    for (let y = anchor.y; y < anchor.y + footprint.height; y += 1) {
      for (let x = anchor.x; x < anchor.x + footprint.width; x += 1) cells.push(roomCellKind(variant, x, y));
    }
  ...
  for (const placement of variant.placements.filter(({ placementId }) => placementId !== ignorePlacementId)) {
    ...
    const otherEnvelope = { x: placement.anchor.x, y: placement.anchor.y, width: otherFootprint.width, height: otherFootprint.height };
    if (layer === 'SET_DRESSING' && placement.layer === 'SET_DRESSING' && roomRectsIntersect(envelope, otherEnvelope)) {
  ...
  return {
    anchor, rotation, footprint, asset, layer,
    allowed: blockers.length === 0,
    message: blockers[0] ?? 'No blocking issue is visible in this advisory preflight.',
  };
}
...
function currentRoomPlacementGhost() {
  ...
  return roomPlacementGhostModel({
    variant, snapshot: state.project.snapshot, asset, anchor: state.roomUi.placementHover,
    rotation: state.roomUi.placementRotation,
    layer: asset?.kind === 'surface' ? 'STRUCTURAL_SURFACE' : 'SET_DRESSING',
  });
}
...
function renderRoomPlacementGhost(model) {
  ...
  const label = document.createElement('span'); label.textContent = `${model.asset?.name ?? 'Missing asset'} · ${model.anchor.x},${model.anchor.y} · ${model.rotation}°`;
  const reason = document.createElement('small'); reason.textContent = `${model.message} Server validation remains authoritative.`;
  ghost.append(cue, label, reason); return ghost;
}

function updateRoomPlacementGhostDom() {
  if (state.workspace !== 'rooms') return;
  const board = elements['workspace-content'].querySelector('[data-room-board]'); if (!board) return;
  board.querySelector('.room-placement-ghost')?.remove();
  const model = currentRoomPlacementGhost();
  if (model?.footprint) board.append(renderRoomPlacementGhost(model));
...
function roomCanvasHintText(variant, ghost = currentRoomPlacementGhost()) {
  ...
  if (state.roomUi.activeTool.startsWith('PAINT_')) return 'Click a cell, or focus it and press Enter/Space, to paint the active class. Save changes keeps all Room edits; Discard changes restores the saved Room. Artwork is dimmed while painting, not removed.';
...
function roomCreationBlockedReason() {
  if (roomMoveTools.hasPending()) return 'Save or discard all Room changes before creating a room or template.';
...
function mayLeaveRoomNavigation(onDiscard = null) {
  if (roomMoveTools.hasPending()) { showToast('Save or discard all Room changes before leaving. Your draft stays here.'); return false; }
...
function renderRoomPlacementPreview(variant, snapshot) {
  ...
  if (!asset) {
    section.append(sectionHeading('Placement preview', 'Select a prop or item from the palette. Its exact image and authored placement rules appear here, and the asset is armed immediately when its image is ready.'));
    return section;
  }
  section.append(sectionHeading(asset.name, 'The selected exact asset remains active so you can place multiple copies. Rotation changes apply to the active placement ghost.'));
  ...
  const active = document.createElement('p'); active.className = 'room-placement-active'; active.setAttribute('role', 'status');
  active.textContent = exactSelectedRoomPaletteAsset(snapshot)?.assetId === asset.assetId
    ? 'Ready to place · choose one or more free room cells.'
    : variant.lifecycle !== 'DRAFT' ? `${variant.lifecycle} room versions are read-only.` : 'Exact image is not ready; placement remains disabled.';
  const close = roomControl('Choose another asset', 'close-preview-asset'); actions.append(active, close); section.append(actions);
  return section;
}
...
function renderRoomPlacement(placement, snapshot) {
  ...
  placed.dataset.roomPositionKey = `${placement.anchor.x}:${placement.anchor.y}:${placement.rotation}`;
...
function renderRoomFindings(variant) {
  variant = currentRoomVariant().variant ?? variant;
  ...
  guidance.textContent = roomMoveTools.hasPending()
    ? `Unsaved changes have not been fully checked. These findings describe saved room v${variant.version}, not this draft. Save changes checks the complete draft before saving.`
    : `These findings belong to exact saved room v${variant.version}. Choose a linked target to locate it; no automatic fix or acceptance is performed.`;
  section.append(guidance);
...
function applyRoomShapeDraftLock(root, variant) {
  if (!roomMoveTools.hasPending()) return;
  for (const action of ['proposal-decide', 'proposal-apply', 'validate', 'finalize', 'fork', 'warning-save']) {
    for (const control of root.querySelectorAll(`[data-room-control="${action}"]`)) { control.disabled = true; control.title = 'Save or discard all Room changes first.'; }
  }
}
...
function renderRoomEditForms(variant, mode = 'all') {
  ...
  const resizeNote = document.createElement('p'); resizeNote.className = 'room-resize-guidance';
  resizeNote.textContent = 'Resize the local Room draft. Content that would be clipped requires confirmation; Save changes keeps all edits together.';
  ...
  const saveIntent = document.createElement('button'); saveIntent.type = 'submit'; saveIntent.textContent = 'Apply intent to draft'; intent.append(saveIntent);
  ...
  const copy = {
    ...
    purpose: ['Room size and intent', 'Resize the saved bounded area and record the design purpose in plain language.'],
  ...
  if (mode === 'shape') section.append(resizeNote, resize);
  else if (mode === 'entrances') section.append(connector);
  else if (mode === 'purpose') section.append(resizeNote, resize, intent);
  else section.append(resizeNote, resize, connector, intent);
  return section;
}
...
function renderRoomViewSwitch(variant) {
  ...
  for (const [view, label] of [['editor', 'Editor'], ['preview', 'Studio preview']]) {
    ...
    button.disabled = view === 'preview' && (roomMoveTools.hasPending() || state.roomMutationPending || Boolean(state.roomUi.pendingPlacementAdd));
  ...
  description.textContent = roomMoveTools.hasPending()
...
function renderRooms(snapshot) {
  ...
  ensureRoomPinnedAssets(variant, snapshot);
  fragment.append(renderRoomPinnedAssetsStatus(variant, snapshot));
  ...
  editor.append(shell); applyRoomShapeDraftLock(editor, variant); applyRoomPinnedAssetsLock(editor, variant, snapshot);
...
function workspaceRenderFingerprint() {
  ...
  return JSON.stringify({
    ...
    sourcesUi: state.workspace === 'sources' ? state.sourcesUi : null,
    ...
    roomUi: state.workspace === 'rooms' ? {
      ...
      placementHover: state.roomUi.placementHover,
      placementGesture: state.roomUi.placementGesture ? {
        projectId: state.roomUi.placementGesture.projectId,
        projectRevision: state.roomUi.placementGesture.projectRevision,
        roomVariantId: state.roomUi.placementGesture.roomVariantId,
        roomVersion: state.roomUi.placementGesture.roomVersion,
        placementId: state.roomUi.placementGesture.placementId,
...
function renderWorkspace({
  ...
  if (state.workspace === 'tasks' && sharedReviewSupported() && !state.cutter) {
    const pending = libraryAllGroups().filter(group => group.pending && (state.taskUi.view !== 'detail' || group.taskId === state.taskUi.selectedTaskId));
    if (pending.length) {
...
function reconcileRoomUi(project) {
  ...
  if (pendingAdd) {
    ...
    if (exactCommit) {
      ...
      state.roomUi.selectedPlacementId = null;
      state.roomUi.placementHover = null;
      showToast('The authoritative reload confirmed the original placement. No duplicate command was sent; the asset remains active.');
  ...
  if (state.roomUi.selectedPaletteAssetId && !exactSelectedRoomPaletteAsset(project.snapshot)) clearRoomPaletteAsset();
...
async function loadProjects(preferredProjectId, { preserveWorkspaceIfUnchanged = false, signal = null, canApply = null } = {}) {
  if (roomMoveTools.hasPending()) return false;
...
async function refresh({ quiet = false, passive = false, background = false } = {}) {
  if (roomMoveTools.hasPending()) { if (!background && !quiet) showToast('Save or discard all Room changes before refreshing.'); return; }
...
function roomCellFromPointer(board, event) {
  const { variant } = currentRoomVariant(); if (!variant || !board?.isConnected) return null;
  const rect = board.getBoundingClientRect();
  if (rect.width <= 0 || rect.height <= 0) return null;
  const x = Math.floor((event.clientX - rect.left) / (rect.width / variant.width));
  const y = Math.floor((event.clientY - rect.top) / (rect.height / variant.height));
...
elements['workspace-content'].addEventListener('pointerdown', (event) => {
  ...
  state.roomUi.selectedConnectorId = null; state.roomUi.selectedFinding = null; clearRoomPaletteAsset(); state.roomUi.placementHover = null;
...
elements['workspace-content'].addEventListener('pointerup', async (event) => {
  ...
  await moveRoomPlacement(placement, gesture.anchor, gesture.rotation, gesture);
});
...
document.addEventListener('keydown', async (event) => {
  ...
  if (event.key === 'Escape') {
    ...
    if (state.roomUi.pendingPlacementAdd) showToast('This placement is not yet confirmed. Choose the same cell again to retry safely; Studio will not create a duplicate.');
  ...
  if (event.key === 'Delete' || event.key === 'Backspace') {
    if (roomMoveTools.isLocked()) { event.preventDefault(); return; }
...
elements['workspace-content'].addEventListener('click', async (event) => {
  ...
  if (previewRotation) {
    const rotation = Number(previewRotation.dataset.assetPreviewRotation);
    state.assetUi.previewRotations[previewRotation.dataset.previewKey] = rotation;
    const selectedAsset = exactSelectedRoomPaletteAsset();
    if (selectedAsset && previewRotation.dataset.previewKey === `room:${selectedAsset.assetId}`) {
      state.roomUi.placementRotation = rotation; state.roomUi.placementHover = null;
    }
    renderWorkspace({ preserveAssetDraft: true, preserveRoomDraft: true });
    return;
  }
...
function renderRoomMoveDisplay({ statusOnly = false } = {}) {
  if (state.workspace !== 'rooms' || state.roomNavigation.route !== 'editor') return;
  captureRoomDomState({ preserveActiveKey: true });
  const root = elements['workspace-content'], { variant } = currentRoomVariant();
  const board = root.querySelector('[data-room-board]');
  if (board && variant && !statusOnly) {
    ...
    const nodes = new Map([...board.querySelectorAll('.room-placement')].map(node => [node.dataset.placementId, node]));
    const placements = roomMoveTools.project(variant).placements;
    const visibleIds = new Set(placements.filter(placement => state.roomUi.layers[placement.layer]).map(placement => placement.placementId));
    for (const [placementId, node] of nodes) if (!visibleIds.has(placementId)) node.remove();
    for (const placement of placements) {
      if (!state.roomUi.layers[placement.layer]) continue;
      const existing = nodes.get(placement.placementId);
      if (existing && existing.dataset.roomPositionKey === `${placement.anchor.x}:${placement.anchor.y}:${placement.rotation}`
          && existing.dataset.selected === String(state.roomUi.selectedPlacementId === placement.placementId)) continue;
  ...
  if (variant) root.querySelector('.room-tool-options')?.replaceWith(renderRoomToolOptions(variant));
  setRoomMutationPending(roomMoveTools.isLocked());
  if (variant) applyRoomShapeDraftLock(root, variant);
  restoreRoomDomState();
  updateRoomPlacementGhostDom();
}

const roomMoveTools = createRoomEditorDraft({
  context() {
    const { variant } = currentRoomVariant();
    if (!variant || !state.project) return null;
    const blocked = manualProjectRefreshActive ? 'Wait for Refresh live status to finish. Nothing was changed.'
      ...
      : state.roomMutationPending && !roomMoveTools.isLocked() ? 'Wait for the current Room operation to finish.'
  ...
  send(request) {
    return api(`/api/projects/${encodeURIComponent(request.projectId)}/rooms/${encodeURIComponent(request.roomVariantId)}/editor-save`, {
      method: 'POST', headers: { 'x-numberdroid-studio-csrf': state.agentAccessCsrf }, body: request.serialized,
      signal: AbortSignal.timeout(15_000),
    });
  },
  async adopt(response, request, previous, draft, { forceRead = false } = {}) {
    if (state.project?.projectId !== request.projectId || currentRoomVariant().variant?.roomVariantId !== request.roomVariantId) throw new Error('Return to the original Room to confirm the saved changes.');
    let room = forceRead ? null : response.value?.roomVariant;
    if (!room) {
      const query = await api(`/api/projects/${encodeURIComponent(request.projectId)}/rooms/${encodeURIComponent(request.roomVariantId)}?includeVersions=false&includeProposals=false`, { signal: AbortSignal.timeout(15_000) });
      if (query.projectId !== request.projectId || query.revision !== response.revision) throw new Error('The exact saved Room is not available yet.');
      room = query.variants?.find(entry => entry.roomVariantId === request.roomVariantId)?.current;
    }
    if (!room || room.roomVariantId !== previous.roomVariantId || room.version !== previous.version + 1
        || room.createdRevision !== response.revision || room.lifecycle !== previous.lifecycle
        || room.contentFingerprint !== response.value.contentFingerprint || !Array.isArray(room.findings)) throw new Error('The exact saved Room identity could not be verified.');
    if (!roomEditorSavedMatches(room, draft)) throw new Error('The saved content did not match the requested Room draft.');
    const entry = currentRoomVariant().entry;
    if (state.project.revision !== request.body.expectedRevision || entry.headVersion !== previous.version) throw new Error('The project changed while confirming the saved Room.');
    if (!entry.versions.some(value => value.version === room.version)) entry.versions.push(room);
    entry.headVersion = room.version; state.project.revision = response.revision;
    if (state.roomUi.pinnedAssets.status === 'ready') state.roomUi.pinnedAssets.key = roomPinnedAssetsKey(roomPinnedAssetsContext(request.projectId, response.revision, room));
    state.roomUi.shapeDraft = null; state.roomUi.shapeConflict = null; state.roomUi.selectedFinding = null;
    if (response.event && !state.activity.some(value => value.eventId === response.event.eventId)) state.activity.push(response.event);
    const option = [...elements['project-select'].options].find(value => value.value === request.projectId);
    if (option) option.textContent = `${state.project.snapshot.project.name} · r${response.revision}`;
    projectLoadGeneration += 1; cancelRoomPreviewLoad(); return true;
  },
  changed(kind) {
    const moveState = roomMoveTools.getState(), variant = currentRoomVariant().variant;
    if (kind === 'discard' || kind === 'saved') state.roomUi.lastShapeEdit = null;
    ...
    state.roomUi.shapeDraft = null; state.roomUi.shapeConflict = null;
    if (kind === 'move' || kind === 'surface' || kind === 'status') renderRoomMoveDisplay({ statusOnly: kind === 'status' });
    else if (state.workspace === 'rooms') renderWorkspace({ preserveRoomDraft: true });
  },
});
...
async function executeRoomMutation({ operation, target, path, body, successMessage, onBeforeReload = null, capturedRequest = null, onFailure = null }) {
  if (['room-shape-set', 'room-placement-add', 'room-placement-remove', 'room-connectors-set', 'room-resize', 'room-intent-set'].includes(operation)) {
    const applied = roomMoveTools.apply(operation, body);
    if (applied) { onBeforeReload?.(); showToast('Unsaved changes. Save changes keeps all Room edits together.'); }
    return applied;
  }
  if (roomMoveTools.hasPending()) { showToast('Save or discard all Room changes before this separate action.'); return false; }
...
function roomManipulationContext(variant, placement = null) {
  return {
    ...
    expectedAssetId: placement?.assetId ?? null,
  };
}

function roomManipulationContextMatches(context, variant, placement = null) {
  return Boolean(context && state.project
    && state.project.projectId === context.projectId
    && state.project.revision === context.projectRevision
    && variant?.roomVariantId === context.roomVariantId
    && variant?.version === context.roomVersion
    && (!context.placementId || (placement?.placementId === context.placementId
      && placement?.assetId === context.expectedAssetId)));
}

async function moveRoomPlacement(placement, anchor, rotation, context = null) {
  const { variant } = currentRoomVariant();
  if (!variant || !placement || (anchor.x === placement.anchor.x && anchor.y === placement.anchor.y && rotation === placement.rotation)) return false;
  const captured = context ?? roomManipulationContext(variant, placement);
  if (!roomManipulationContextMatches(captured, variant, placement)) {
    showToast('ROOM_CONTEXT_CHANGED: The placement was not changed because its rendered room context is stale.');
    return false;
  }
  const displayed = displayedRoomVariant();
  const model = roomPlacementGhostModel({ variant: displayed, snapshot: state.project.snapshot, asset: exactRoomAsset(placement), anchor, rotation, layer: placement.layer, ignorePlacementId: placement.placementId });
  if (!model.allowed) { showToast(model.message); return false; }
...
async function removeRoomPlacement(placement, context = null) {
  const { variant } = currentRoomVariant(); if (!variant || !placement) return false;
  const captured = context ?? roomManipulationContext(variant, placement);
  if (!roomManipulationContextMatches(captured, variant, placement)) {
    showToast('ROOM_CONTEXT_CHANGED: The placement was not removed because its rendered room context is stale.'); return false;
  }
  if (!window.confirm(`Remove placement ${placement.placementId}? The prior immutable room version remains available.`)) return false;
  ...
  return executeRoomMutation({
    operation: 'room-placement-remove', target: `${placement.placementId}:${captured.roomVersion}`,
    path: `${basePath}/placements-remove`,
    body: { expectedRoomVariantVersion: captured.roomVersion, placements: [{ placementId: placement.placementId, expectedAssetId: captured.expectedAssetId }] },
    successMessage: 'Placement removed in a new room version.',
    onBeforeReload: () => { state.roomUi.selectedPlacementId = null; },
  });
}

async function addRoomPlacement(asset, anchor, rotation) {
  const variant = displayedRoomVariant(); if (!variant || !asset || !state.project) return false;
  const context = roomManipulationContext(variant);
  const layer = asset.kind === 'surface' ? 'STRUCTURAL_SURFACE' : 'SET_DRESSING';
  const model = roomPlacementGhostModel({ variant, snapshot: state.project.snapshot, asset, anchor, rotation, layer });
  ...
  return roomMoveTools.apply('room-placement-add', { placements: [{
    placementId: stableUiId('placement', asset.name), assetId: asset.assetId, assetVersion: asset.assetVersion,
    metadataVersion: asset.metadataVersion, layer, anchor: { ...anchor }, rotation,
...
elements['workspace-content'].addEventListener('change', (event) => {
  ...
  if (roomSelect) {
    if (roomMoveTools.hasPending()) { roomSelect.value = state.roomUi.selectedRoomVariantId; showToast('Save or discard all Room changes before switching Rooms.'); return; }
    ...
    clearRoomPaletteAsset(); state.roomUi.previewAssetId = null; state.roomUi.selectedProposalId = null; state.roomUi.dirty = false; state.roomUi.conflict = null;
...
elements['workspace-content'].addEventListener('submit', async (event) => {
  ...
  const data = new FormData(form); const projectId = state.project.projectId; const variant = displayedRoomVariant();
  ...
  if (form.dataset.roomForm === 'shape-coordinates') {
    ...
    try {
      ...
      roomMoveTools.apply('room-shape-set', { voidCells, blockedCells });
...
elements['workspace-content'].addEventListener('click', (event) => {
  ...
  if (view) {
    ...
    if (view.dataset.roomView === 'preview') {
      if (roomMoveTools.hasPending() || state.roomMutationPending || state.roomUi.pendingPlacementAdd) return;
      captureRoomDomState({ preserveActiveKey: true });
...
elements['workspace-content'].addEventListener('click', async (event) => {
  ...
  if (roomMoveTools.isLocked() && action !== 'zoom') return;
  if (action === 'editor-save') { await roomMoveTools.save(); return; }
  if (action === 'editor-discard') {
    if (roomMoveTools.hasPending() && window.confirm('Discard all unsaved Room changes? The saved Room remains unchanged.')) roomMoveTools.discard();
    return;
  }
  ...
  if (action === 'editor-tool') {
    ...
    state.roomUi.activeTool = tool; state.roomUi.dockPanel = 'tool'; clearRoomPaletteAsset(); state.roomUi.selectedPlacementId = null;
    state.roomUi.placementHover = null; state.roomUi.placementGesture = null; state.roomUi.placementRotation = 0;
    renderWorkspace({ preserveRoomDraft: true, preserveRoomCanvas: true }); updateRoomPlacementGhostDom(); settleRoomEditorControlFocus(`room-tool-${tool}`); return;
  }
  ...
  if (action === 'palette-asset') {
    ...
    state.roomUi.placementHover = null;
    const previewReady = control.querySelector('.asset-preview.ready')?.dataset.previewState === 'READY';
    if (!asset || variant?.lifecycle !== 'DRAFT' || !previewReady) {
      clearRoomPaletteAsset();
      if (asset) state.roomUi.previewAssetId = asset.assetId;
      showToast(variant?.lifecycle !== 'DRAFT'
        ? `${variant?.lifecycle ?? 'This'} room version is read-only.`
        : 'Wait for the exact image preview to load, then choose the asset again.');
      renderWorkspace({ preserveRoomDraft: true }); return;
    }
  ...
  if (action === 'placement-select') {
    ...
    if (state.roomUi.activeTool === 'CLEAR' && control.closest('[data-room-board]')) {
      const placement = variant?.placements.find(({ placementId }) => placementId === control.dataset.placementId);
      if (!placement || state.roomMutationPending) return;
      await removeRoomPlacement(placement); return;
    }
  ...
  if (action === 'cell') {
    if (state.roomUi.suppressCanvasClick) return;
    ...
    if (state.roomUi.activeTool.startsWith('PAINT_')) {
      ...
      roomMoveTools.apply('room-shape-set', { voidCells: draft.voidCells, blockedCells: draft.blockedCells }); return;
    }
    ...
    if (state.roomUi.activeTool === 'SELECT' && state.roomUi.selectedPlacementId) {
  ...
  if (['move-placement', 'rotate-placement'].includes(action)) {
    ...
    await moveRoomPlacement(placement, anchor, rotation); return;
  }
...
elements['workspace-content'].addEventListener('click', (event) => {
  ...
  if (open.dataset.overviewOpen === 'tasks') {
    ...
    cancelPinnedAssetsOnWorkspaceExit('tasks');
...
elements['workspace-nav'].addEventListener('click', (event) => {
  ...
  cancelPinnedAssetsOnWorkspaceExit(link.dataset.workspace);
...
elements['project-select'].addEventListener('change', () => {
  if (roomMoveTools.hasPending()) { elements['project-select'].value = state.project?.projectId ?? ''; showToast('Save or discard all Room changes before switching projects.'); return; }
  ...
  if (state.roomUi.pendingPlacementAdd) {
    elements['project-select'].value = state.project?.projectId ?? '';
    showToast('A placement is not yet confirmed. Choose its original cell again before switching projects.'); return;
  }
...
elements['agent-access-select'].addEventListener('change', () => {
  if (roomMoveTools.hasPending()) { renderAgentAccess(); showToast('Save or discard Room changes before changing agent access.'); return; }
...
elements['workspace-content'].addEventListener('input', (event) => {
  const search = event.target.closest('[data-sources-search]');
  if (!search) return;
  state.sourcesUi.search = search.value;
  rerenderSourcesWorkspace({
    focus: '[data-sources-search]',
...
elements['workspace-content'].addEventListener('change', (event) => {
  const attention = event.target.closest('[data-sources-attention]');
  if (!attention) return;
  state.sourcesUi.attention = attention.value;
...
elements['workspace-content'].addEventListener('click', (event) => {
  ...
  if (tab) {
    ...
    state.sourcesUi.search = '';
    rerenderSourcesWorkspace({ focus: `[data-sources-tab="${state.sourcesUi.tab}"]` });
    return;
  }
  if (event.target.closest('[data-source-import-toggle]')) {
    state.sourcesUi.importOpen = !state.sourcesUi.importOpen;
...
if (visualFixture) {
  ...
  window.__numberdroidStudioVisualTest = Object.freeze({
    ...
    roomEditorDraftState() {
      const saved = currentRoomVariant().variant;
      return { state: roomMoveTools.getState(), saved: structuredClone(saved), displayed: structuredClone(roomMoveTools.project(saved)) };
    },
    roomSurfacePlanState() {
      return structuredClone(roomSurfaceTools.getState());
    },
    ...
    async roomPlacementVisualRotationEvidence(assetId = 'asset.transfer-apparatus-cp45') {
      const asset = currentAssetLibrary().assets.find((candidate) => candidate.assetId === assetId);
      ...
      try {
        for (const rotation of [0, 90, 180, 270]) {
          const span = roomAssetSpan(asset, rotation);
          host.style.width = `${span.width * 80}px`; host.style.height = `${span.height * 80}px`;
          const visual = roomPlacementVisual(asset, rotation); host.replaceChildren(visual);
          const image = visual.querySelector('img');
          if (image && !image.complete) {
            await new Promise((resolveImage) => {
              const timeout = setTimeout(resolveImage, 5_000);
              const settle = () => { clearTimeout(timeout); resolveImage(); };
              image.addEventListener('load', settle, { once: true });
              image.addEventListener('error', settle, { once: true });
            });
          }
    ...
    async exerciseRoomShapeRefresh() {
      ...
      before?.focus();
      const beforeVoidCount = displayedRoomVariant()?.voidCells?.length ?? 0;
      ...
      return {
        beforeVoidCount,
        afterVoidCount: displayedRoomVariant()?.voidCells?.length ?? 0,
        dirty: roomMoveTools.getState().dirty,
    ...
    async exerciseRoomShapeConflict() {
      ...
      await roomMoveTools.save();
      await publishVisualEvidence();
      return {
        dirty: roomMoveTools.getState().dirty,
        conflict: roomMoveTools.getState().message,
    ...
    roomShapeState() {
      ...
      return {
        dirty: roomMoveTools.getState().dirty,
        conflict: roomMoveTools.getState().message.startsWith('Not saved:') ? roomMoveTools.getState().message : null,
        voidCells: structuredClone(displayed?.voidCells ?? []),
        blockedCells: structuredClone(displayed?.blockedCells ?? []),
      };
    },
    async exerciseRoomCoordinateOverlapRejection() {
      const form = document.querySelector('[data-room-form="shape-coordinates"]');
      const before = structuredClone(displayedRoomVariant()); if (!form || !before) return null;
      const beforeDirty = roomMoveTools.getState().dirty;
      ...
      return {
        arraysUnchanged: JSON.stringify(displayedRoomVariant()?.voidCells) === JSON.stringify(before.voidCells)
          && JSON.stringify(displayedRoomVariant()?.blockedCells) === JSON.stringify(before.blockedCells),
        dirtyUnchanged: roomMoveTools.getState().dirty === beforeDirty,
        message: elements.toast.textContent,
      };
    },
...
elements['workspace-content'].addEventListener('submit', async (event) => {
  ...
  try {
    ...
    state.sourceDraft = null; state.resumingIntakeId = null; state.sourcesUi.importOpen = false; resetSourceIntakeForm();
    await loadProject(operationProjectId);
...
elements['workspace-content'].addEventListener('click', async (event) => {
  ...
  if (resume) {
    ...
    state.sourceDraft = null;
    state.sourcesUi.tab = 'images';
    state.sourcesUi.importOpen = true;
    renderWorkspace();
...
elements['demo-button'].addEventListener('click', async () => {
  if (roomMoveTools.hasPending()) { showToast('Save or discard all Room changes first.'); return; }
