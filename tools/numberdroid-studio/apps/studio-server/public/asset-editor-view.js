...
export function createAssetEditorView(state) {
  ...
  const title = el('h2'); title.dataset.assetEditorTitle = ''; heading.append(title, el('p', 'asset-editor-intro', embedded ? 'Edit the Assembly’s owned blocking. Back to Assembly keeps this draft, including an unfinished outline; save from the Assembly.' : 'Define what the image is and the space it uses. Your Save adds a draft Asset directly to the Library.'));
  const back = button(state.initial.returnLabel ?? 'Back to Library', 'back'); back.className = 'studio-back-button secondary'; root.append(back); header.append(heading); root.append(header);
  ...
  for (const [tool, name] of [['select', 'Select'], ['polygon', 'Polygon'], ['rectangle', 'Rectangle'], ['oval', 'Oval'], ['insert', 'Point +'], ['remove', 'Remove'], ['grid', 'Grid'], ['undo', 'Undo'], ['redo', 'Redo']]) { const b = button(name, 'tool', tool); b.dataset.assetEditorTool = tool; rail.append(b); } if (!embedded) rail.append(button('Save work', 'save'));
  ...
  const footer = el('footer', 'asset-editor-footer'); const saved = el('span'); saved.dataset.assetEditorSavedState = ''; footer.append(saved, button('Close polygon', 'finish-polygon')); if (!embedded) footer.append(button('Save Asset', 'save')); root.append(footer);
  return root;
}
