...
export function createAssetEditorView(state) {
  const root = el('section', 'asset-editor'); root.dataset.assetEditor = state.instanceId;
  const header = el('header', 'asset-editor-header'); const heading = el('div'); heading.append(el('p', 'eyebrow', state.context.assetVersion ? 'Library / Edit Asset' : 'Saved image / Create Asset'));
  const title = el('h2'); title.dataset.assetEditorTitle = ''; heading.append(title, el('p', 'asset-editor-intro', 'Define what the image is and the space it uses. Your Save adds a draft Asset directly to the Library.'));
  header.append(heading, button(state.initial.returnLabel ?? 'Back to Library', 'back')); root.append(header);
  const tabs = el('nav', 'asset-editor-tabs'); tabs.setAttribute('aria-label', 'Asset editor views'); tabs.append(button('Placement & blocking', 'view', 'edit'), button('Properties', 'view', 'properties'), button('Preview shapes', 'view', 'preview')); root.append(tabs);
  ...
  for (const [tool, name] of [['select', 'Select'], ['polygon', 'Polygon'], ['rectangle', 'Rectangle'], ['oval', 'Oval'], ['insert', 'Point +'], ['remove', 'Remove'], ['grid', 'Grid'], ['undo', 'Undo'], ['redo', 'Redo']]) { const b = button(name, 'tool', tool); b.dataset.assetEditorTool = tool; rail.append(b); } rail.append(button('Save work', 'save'));
  ...
  const image = svg('image', { href: state.initial.previewUrl, x: 0, y: 0, width: state.initial.pixelSize.width, height: state.initial.pixelSize.height }); image.dataset.assetEditorArtwork = ''; canvas.append(image);
  ...
  main.append(el('p', 'asset-editor-legend', 'Blocking · blue dashed placement bounds · cyan alignment anchor. Coordinates use image pixels.'));
  ...
  const footer = el('footer', 'asset-editor-footer'); const saved = el('span'); saved.dataset.assetEditorSavedState = ''; footer.append(saved, button('Close polygon', 'finish-polygon'), button('Save Asset', 'save')); root.append(footer);
  return root;
}
function inspectorContent(state) {
  ...
  if (state.panel === 'placement') {
    ...
    const anchor = el('div', 'asset-editor-coordinates'); anchor.append(number('anchor.x', s.anchor.x, 'X (px)'), number('anchor.y', s.anchor.y, 'Y (px)')); box.append(anchor, button('Set anchor on canvas', 'tool', 'anchor'), el('h3', '', 'Image scale'));
    box.append(number('scale.x', s.unitsPerPixel.x, 'Project units per image pixel · X'), number('scale.y', s.unitsPerPixel.y, 'Project units per image pixel · Y'), el('p', 'asset-editor-note', 'Bounds change positioning space. Scale changes the physical size of artwork and shapes together.'));
    return box;
  }
...
export function syncAssetEditorCanvas(root, state) {
  ...
  const nodes = [];
  for (const region of s.blockingRegions) { const selected = region.regionId === state.selectedRegionId; const shape = shapeNode(region.shape, { class: `asset-editor-region${selected ? ' selected' : ''}`, 'data-asset-editor-region': region.regionId }); nodes.push(shape);
    if (!selected) continue; const r = region.shape; const radius = 5 / scale;
    if (r.kind === 'polygon') r.points.forEach((p, i) => { nodes.push(svg('circle', { cx: p.x, cy: p.y, r: radius, class: `asset-editor-point${state.selectedPoint === i ? ' selected' : ''}`, 'data-asset-editor-point': i, 'data-asset-editor-region': region.regionId })); const n = svg('text', { x: p.x + radius * 1.8, y: p.y - radius * 1.5, 'font-size': 11 / scale, class: 'asset-editor-point-number' }); n.textContent = String(i + 1); nodes.push(n); });
    else for (const edge of ['nw', 'n', 'ne', 'e', 'se', 's', 'sw', 'w']) { const x = edge.includes('w') ? r.x : edge.includes('e') ? r.x + r.width : r.x + r.width / 2; const y = edge.includes('n') ? r.y : edge.includes('s') ? r.y + r.height : r.y + r.height / 2; nodes.push(svg('rect', { x: x - radius, y: y - radius, width: radius * 2, height: radius * 2, class: 'asset-editor-handle', 'data-asset-editor-handle': edge, 'data-asset-editor-region': region.regionId })); }
  }
...
export function updateAssetEditorView(root, state, { inspector = true } = {}) {
  ...
  root.querySelector('[data-asset-editor-saved-state]').textContent = assetEditorDirty(state) ? 'Unsaved changes · Save creates a draft Asset version' : state.context.assetVersion ? `Saved Asset v${state.context.assetVersion} · metadata v${state.context.metadataVersion}` : 'New Asset · not saved yet';
  ...
  if (state.view === 'preview' && (inspector || !root.querySelector('[data-asset-editor-view="preview"]').childElementCount)) { const preview = root.querySelector('[data-asset-editor-view="preview"]'); const picture = svg('svg', { viewBox: `${state.frame?.x ?? 0} ${state.frame?.y ?? 0} ${state.frame?.width ?? state.initial.pixelSize.width} ${state.frame?.height ?? state.initial.pixelSize.height}`, role: 'img', 'aria-label': 'Read-only authored blocking geometry' }); picture.append(svg('image', { href: state.initial.previewUrl, width: state.initial.pixelSize.width, height: state.initial.pixelSize.height })); for (const r of state.model.spatial.blockingRegions) picture.append(shapeNode(r.shape, { class: 'asset-editor-region' })); const note = el('aside'); note.append(el('h3', '', 'Geometry inspection · read-only'), el('p', '', 'This shows your exact current draft. It does not simulate character size, pathfinding or game movement. Nothing is saved, accepted or published.')); for (const r of state.model.spatial.blockingRegions) note.append(el('p', '', `${r.name} · ${r.shape.kind}`)); note.append(button('Return to editing', 'view', 'edit')); preview.replaceChildren(picture, note); }
