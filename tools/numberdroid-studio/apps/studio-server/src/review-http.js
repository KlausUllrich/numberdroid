...
export async function handleReviewHttp({ request, response, url, studioService, humanUiCsrfToken, signal,
  ...
  else if (action === 'feedback') { type = 'review.feedback.save'; assertExactKeys(body, new Set([...common, 'summary', 'itemComments', 'confirmed']), 'Review feedback'); }
  else if (action === 'accept') { type = 'review.accept'; assertExactKeys(body, new Set([...common, 'selectedItemIds', 'confirmed']), 'Review acceptance'); }
  else if (action === 'discard') { type = 'review.discard'; assertExactKeys(body, new Set([...common, 'confirmed']), 'Review discard'); }
