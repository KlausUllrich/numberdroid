...
SQLite/content-addressed persistence and the official MCP transport are the accepted operational path. Checkpoint 2A added source intake/review and its audit/recovery tables. Accepted Checkpoint 2B adds the preview package, durable atlas jobs, job resources, and an in-process worker without changing the inward dependency boundary. The Checkpoint 2C candidate adds slice-bound V2 assets, durable asset proposals, schema v9, feature-gated asset MCP/query surfaces, and a normalized portable project bundle. The Checkpoint 3 candidate adds pure room/archetype validation, immutable room versions, schema v10 normalized room records, a visible room/hallway designer, a placement-only agent proposal seam, and portable schema v2 only when room semantics exist. The Checkpoint 4 candidate adds schema v11 isolated task documents/heads, immutable branch/timeline/review/merge/revert ledgers, human task control, semantic comparison, validated atomic replay to main, compensating revert, and a task-bound 30-tool/five-resource MCP surface. All three candidates remain subject to one combined user gate. The JSON adapter remains only for protected 1A regression and migration. The combined `studio-server` UI/service/worker process is an accepted transitional packaging choice, not the final standalone packaging model.
...
Accepted Checkpoint 1 tables include `projects`, `revisions`, `revision_parents`, `activity_events`, `aggregate_versions`, `projections`, `idempotency_records`, `grants`, `migration_runs`, `artifacts`, `artifact_references`, `cas_gc_marks`, `host_bindings`, and `human_agent_access_operations`, plus the migration ledger maintained by the runner. Accepted schema v6 adds `source_intakes` and the original final-attempt ledger. Accepted schema v8 adds `jobs`, `job_events`, immutable creator authority and output-byte accounting, and extends `agent_attempts` with atomic `AUTHORIZED` control records. The 2C candidate schema v9 adds exact slice bindings; immutable asset versions/findings/heads/tags; durable proposal items/findings/decisions/applications; and sanitized bundle-import/history records. The Checkpoint 3 candidate schema v10 adds archetype versions/heads/rules, room versions/heads/intent/connectors/placements/findings/warning dispositions, and room proposal items/findings/decisions/applications with exact asset-version foreign keys. Checkpoint 4 schema v11 adds `agent_tasks`, immutable `task_branch_revisions` and `task_timeline_events`, versioned immutable `task_reviews`, and immutable `task_merges` / `task_reverts`. Migration 0007 is pinned to `aa951c02158f76f6343819271b78816e211bfe3015cc9f4f979947a075ef25e9`, migration 0008 to `2323dafbef16e418b752ba1602c6d62c1260f00935212358980e6c3e90936730`, migration 0009 to `e387c3e56fb0bb03bd14743c6a7c7a6baad230c02dde8f158e485e25776e7175`, migration 0010 to `99d12a3a7ee7572dd9386bd183fb847631ceab0490b0190e3ba5f1b339cfd40e`, and migration 0011 to `f6ed508f3098e6cdeb3dca2af0a9be7baca12c18fcd9d518f75f4f353242639d`. Projection tables are rebuildable from events plus verified snapshots.
