...
The exact PNG crop decoder accepts non-interlaced 8-bit RGB, RGBA and indexed-color inputs. Indexed input requires one 1–256-entry `PLTE` before `IDAT`; optional `tRNS` must follow the palette and precede image data, with at most one alpha byte per entry. Missing alpha entries (including an empty table) are opaque. Each decoded index must name an existing entry. Filters operate on one-byte indices before exact RGBA expansion, retaining RGB even at zero alpha. Packed 1/2/4-bit palettes, interlace, other color types/depths and RGB transparency chunks remain unsupported. CRC, raw chunk-name/order checks, input/dimension/inflate limits and an explicit expanded-RGBA byte ceiling apply before allocation. Canonical RGBA output encoding, processor identity, saved output hashes and processing-result re-encoding checks are unchanged. This input-compatibility extension does not migrate state or confer review/adoption authority.
