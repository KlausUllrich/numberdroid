...
- **AST-013.** Human slice choices MUST show the authored cut name when present, with a readable ordinal fallback for unnamed cuts, while retaining the stable canonical slice ID as secondary, copyable provenance. The approved [Cutter Editor Design](CUTTER_EDITOR_DESIGN.md) refines the earlier ordinal-only labeling rule; names never replace stable identity.
...
4. Rectangle validation rejects unsafe integers, duplicates, overlap, out-of-bounds geometry, invalid pivots/remaps, more than 64 rectangles, more than 64 Mi output pixels, or a canonical slice exceeding 16 MiB before job creation. Cutting accepts approved non-interlaced 8-bit RGB/RGBA PNG sources and, through the bounded 2026-09-22 compatibility extension, indexed-color PNG sources with a validated palette and optional palette transparency. Unsupported depths/color types, interlace, RGB transparency chunks and malformed inputs fail closed. Palette decoding preserves exact RGBA samples; existing canonical output bytes, processor identity and historical Checkpoint 2B acceptance are unchanged.
