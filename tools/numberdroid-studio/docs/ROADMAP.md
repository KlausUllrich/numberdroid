...
**Status: 2A user-accepted on 2026-08-21; 2B is an implemented and CI/browser-verified candidate pending explicit user acceptance; 2C remains blocked.** Draft PR #135 remains open, draft, and unmerged. Candidate verification does not imply release, publication, provider, room, export, or 2C authority.
...
The first 2B user gate was rejected at file selection and repaired through the Chrome UnicodeSets source-ID fix. The next live gate then exposed a cutter defect: after grid proposal, the five-second refresh jumped the image's local scrollbar to the top. Unconditional passive rerendering and adjacent focus, duplicate-poll, stale-context, and active-drag races were repaired in `dd2a4ff2ebdb856af32d5339b996fed9dd69ad2d`; `04d876da750f348e24de9420be1ff59c349bc092` is the selector-only evidence follow-up. The former `a3d9a18e5deb0d9049e7ba6f879ad22eea1dfb29` candidate is superseded and rejected for the user-reported scroll defect. Run `32568108922` is the current green replacement. The user should repeat only the failed scrollbar/focus gate; already-passed import, source-review, preview, and exact-grid gates stand. If the repair passes, the walkthrough resumes at the next unverified cutter action and continues through preview/job control, commit, restart, and persistence. Acceptance will decide cutter accuracy, job visibility/control, remap clarity, and preview usability only.

2B exit decision: **pending explicit user acceptance.** Frozen and CI verification passes 109/109 Node tests. GitHub Actions run `32568108922` passed Studio job `97019592824` and root job `97019592908` under Chrome `151.0.7922.137`. Its valid 2B artifact is `9474639509`, 2,838,857 bytes, `sha256:86366bc207fe081effeb8825b7cc4586026654fbe1c83ce026e1e12e6fc7ebd8`, expiring `2026-09-05T10:40:37Z`. Both 1440×900 and 1060×900 observations prove unchanged passive refresh keeps the same DOM, focused `top=5`, local X321/Y417, and window position; necessary render produces exact y5/h621 geometry without losing scroll; poll ownership and stale identity are bounded; drag replacement is deferred until settlement with inspector/SVG agreement; close/reopen creates a new Fit instance at 0,0; runtime/network errors remain zero. Failed run `32567878956` and 2B artifact `9474582144` are diagnostic only because a stale generated-ID selector failed after the product assertions passed. Only the repeat user decision remains pending; 2C, merge, release, providers, rooms, export, materialization, and publication stay blocked.
