...
function requireNamespacedId(value, label) {
  const result = requireString(value, label, { max: 128 });
  invariant(result.trim() === result && NAMESPACED_ID_PATTERN.test(result),
    'TASK_CANDIDATE_INPUT_INVALID', `${label} must be a lowercase dotted identifier.`, { field: label });
  return result;
}
...
function requireLogicalPath(value, label) {
  const path = requireString(value, label, { max: 1_024 });
  invariant(!path.startsWith('/') && !path.startsWith('\\') && !/^[A-Za-z]:/.test(path)
    && !path.includes('\\') && !path.includes('//'),
  'TASK_CANDIDATE_PATH_UNSAFE', `${label} must be a normalized candidate-relative path.`, { field: label });
  const segments = path.split('/');
  invariant(segments.every((segment) => segment !== '.' && segment !== '..'
    && LOGICAL_PATH_SEGMENT_PATTERN.test(segment) && !segment.endsWith('.')),
  'TASK_CANDIDATE_PATH_UNSAFE', `${label} contains an unsafe path segment.`, { field: label });
  return path;
}
...
function normalizeOutputBytes(values) {
  ...
  const outputs = values.map((raw, index) => {
    ...
    return {
      logicalPath: requireLogicalPath(output.logicalPath, `candidate.outputs[${index}].logicalPath`),
...
function normalizeOutputs(values, manifest) {
  const outputs = normalizeOutputBytes(values);
...
export function taskCandidateOutputClosureSha256(values) {
  return outputClosureFingerprint(normalizeOutputBytes(snapshotPlainData(values, 'outputs')));
}
...
export function validateTaskCandidatePayload(value) {
  ...
  invariant(manifest.artifacts.length === 0,
    'TASK_CANDIDATE_ARTIFACT_UNSUPPORTED', 'Task-candidate v1 is a complete text-output closure and cannot reference external artifacts.');
  ...
  invariant(requireHash(payload.outputClosureFingerprint, 'candidate.outputClosureFingerprint') === closure,
    'TASK_CANDIDATE_OUTPUT_MISMATCH', 'The payload does not bind the exact output closure.');
  const normalized = {
...
export function createTaskCandidatePreview(value) {
  ...
  const preview = {
    ...
    facts: normalizePreviewFacts(input.facts),
    steps: normalizePreviewSteps(input.steps),
  };
...
function normalizeDiffOutputs(values) {
  ...
  return values.map((raw, index) => {
    ...
    return {
      logicalPath: requireLogicalPath(output.logicalPath, `diff.outputs[${index}].logicalPath`),
...
export function createTaskCandidateDiff(value) {
  ...
  const diff = {
    ...
    candidateFingerprint: input.candidateFingerprint,
    changes: normalizeDiffChanges(input.changes),
    outputs: normalizeDiffOutputs(input.outputs),
  };
...
function normalizeBridgeReceipt(value, candidateFingerprint) {
  ...
  return {
    ...
    status: 'VALIDATED',
    bridge: {
      id: requireNamespacedId(bridge.id, 'submission.engineBridgeReceipt.bridge.id'),
...
export function validateTaskCandidateSubmission(value) {
  ...
  const expectedOutputDiff = candidate.outputs.map(({ logicalPath, sha256: afterSha256 }) => ({
    logicalPath,
    operation: 'ADD',
    beforeSha256: null,
    afterSha256,
...
export function createTaskCandidateSubmission(value) {
  ...
  const candidate = validateTaskCandidatePayload(input.candidate);
  const preview = validateTaskCandidatePreview(input.preview);
  const diff = validateTaskCandidateDiff(input.diff);
  const submission = {
    schemaVersion: TASK_CANDIDATE_SCHEMA_VERSION,
    kind: TASK_CANDIDATE_SUBMISSION_KIND,
    ...
    preview,
    diff,
    compilerPins: normalizeCompilerPins(input.compilerPins),
    engineBridgeReceipt: normalizeBridgeReceipt(input.engineBridgeReceipt, candidate.candidateFingerprint),
    status: 'WAITING_FOR_HUMAN_REVIEW',
    authority: {
      reviewDecision: 'NOT_AUTHORIZED',
      merge: 'NOT_AUTHORIZED',
