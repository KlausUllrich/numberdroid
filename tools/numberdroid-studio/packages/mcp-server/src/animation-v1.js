...
export function validateAnimationNegotiation(value, projectId) {
  ...
  if (!value || typeof value !== 'object' || Object.keys(value).length !== keys.length || keys.some(key => !Object.hasOwn(value,key))
    || value.schemaVersion !== 1 || value.profile !== 'animation-v1' || value.projectId !== projectId || ![17, 18, 19].includes(value.storeSchemaVersion)
    || value.sharedHead !== true || value.toolCount !== 25 || value.resourceTemplateCount !== 7) {
    throw new StudioError('ANIMATION_NEGOTIATION_REQUIRED', 'Animation profile requires positive shared-head SQLite v17/v18/v19 gateway negotiation.');
  }
