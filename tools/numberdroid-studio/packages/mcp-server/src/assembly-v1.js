...
export function validateAssemblyNegotiation(value, projectId) {
  ...
  if (!value || typeof value !== 'object' || Object.keys(value).length !== keys.length || keys.some(key => !Object.hasOwn(value, key))
    ...
    || ![16, 17].includes(value.storeSchemaVersion) || value.sharedHead !== true || value.toolCount !== 21 || value.resourceTemplateCount !== 5) {
    throw new StudioError('ASSEMBLY_NEGOTIATION_REQUIRED', 'Assembly profile requires positive shared-head compatible SQLite v16/v17 gateway negotiation.');
  }
