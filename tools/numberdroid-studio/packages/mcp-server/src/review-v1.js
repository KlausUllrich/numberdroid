...
export function validateReviewNegotiation(value, projectId) {
  ...
  if (!value || typeof value !== 'object' || Object.keys(value).length !== keys.length || keys.some(key => !Object.hasOwn(value, key))
    || value.schemaVersion !== 1 || value.profile !== 'review-v1' || value.projectId !== projectId || ![18, 19].includes(value.storeSchemaVersion)
    || value.sharedHead !== true || value.toolCount !== 27 || value.resourceTemplateCount !== 8) {
    throw new StudioError('REVIEW_NEGOTIATION_REQUIRED', 'Shared Review requires positive shared-head SQLite v18/v19 gateway negotiation.');
  }
