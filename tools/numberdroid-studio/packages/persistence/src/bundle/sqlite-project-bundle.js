...
export function projectSqlitePortableDocument({ projectStore, projectId }) {
  ...
  return projectStore.workspace.readTransaction((database) => {
    ...
    const hasReviewHistory = Number(database.prepare('PRAGMA user_version').get().user_version) >= 18
      && database.prepare('SELECT 1 FROM review_versions WHERE project_id=? LIMIT 1').get(projectId);
    const semanticReviewHistory = database.prepare('SELECT command_type,revision_json FROM revisions WHERE project_id=?').all(projectId)
      .some(record => record.command_type.startsWith('review.') || (() => {
        const revision = parseJson(record.revision_json, 'revisions.revision_json');
        return revision.command?.type?.startsWith('review.') || revision.snapshot?.reviewLibrary?.groups?.length;
      })());
    invariant(!hasReviewHistory && !semanticReviewHistory && !(snapshot.reviewLibrary?.groups?.length), 'REVIEW_BUNDLE_UNSUPPORTED', 'Portable bundles cannot yet preserve shared Review history. Use a verified workspace backup instead.');
