...
export async function captureAssemblyEditor({ devtools, sessionId, reopen = false, captureCheckpoint = async () => {} }) {
  ...
  async function captureReview() {
    ...
    const root = '[data-review-workspace]';
    const control = suffix => `${root} ${suffix}`;
    await waitFor(`Boolean(document.querySelector(${JSON.stringify(control('[data-review-action="accept"]'))})) && !document.querySelector(${JSON.stringify(control('[data-review-action="accept"]'))})?.disabled`, 'Resolved Assembly proposal');
    await evaluate(`document.querySelector(${JSON.stringify(root)}).scrollIntoView({block:'start'})`); await settle();
    const before = await project();
    const preview = () => evaluate(`(() => {const root=document.querySelector(${JSON.stringify(root)}),canvas=root.querySelector('[data-review-canvas] svg'),feedback=root.querySelector('[data-review-input="summary"]');const r=n=>{const b=n.getBoundingClientRect();return[b.x,b.y,b.width,b.height]};return{frame:canvas?.getAttribute('viewBox'),bounds:r(root.querySelector('[data-review-canvas]')),images:[...canvas.querySelectorAll('[data-assembly-image-key]')].map(n=>({componentId:n.dataset.assemblyComponent,key:n.dataset.assemblyImageKey,transform:n.getAttribute('transform'),renderedHeight:n.querySelector('image')?.getBoundingClientRect().height??0})),feedback:feedback?.value??null,feedbackBounds:feedback?r(feedback):null,feedbackFont:feedback?parseFloat(getComputedStyle(feedback).fontSize):null,changes:[...root.querySelectorAll('.review-change-summary')].map(n=>n.innerText).join(' '),selection:[...root.querySelectorAll('[data-review-presentation]')].map(n=>[n.dataset.reviewPresentation,n.value]),technicalOpen:[...root.querySelectorAll('details')].some(n=>n.open)};})()`);
    ...
    assert(Math.max(...initial.images.map(image=>image.renderedHeight))>=initial.bounds[3]*.4,'The Assembly must be large enough to inspect inside its preview canvas');
    ...
    await click(control('[data-review-focus="side:current"]'));
    ...
    await click(control('[data-review-focus="side:proposed"]'));
    for (const [key, value] of [['variantId', 'variant.copper'], ['stateId', 'state.ready']]) {
      await evaluate(`(() => {const control=document.querySelector(${JSON.stringify(root)}).querySelector('[data-review-presentation="'+${JSON.stringify(key)}+'"]');control.value=${JSON.stringify(value)};control.dispatchEvent(new Event('change',{bubbles:true}));})()`);
      await waitFor(`!document.querySelector(${JSON.stringify(control('[data-review-action="accept"]'))})?.disabled`, 'Review choice preview');
    }
    ...
    await click(control('[data-review-action="request-changes"]'));
    assert.equal(await evaluate(`document.querySelector('[data-review-action="save-feedback"]').disabled`),true,'Blank feedback is blocked');
    const editing=await preview(); assert(editing.feedbackBounds[2]>=(await evaluate('innerWidth')>=1400?300:260)&&editing.feedbackBounds[3]>=118&&editing.feedbackFont>=14,JSON.stringify(editing));
    ...
    await evaluate(`(() => {const field=document.querySelector(${JSON.stringify(control('[data-review-input="summary"]'))});field.focus({preventScroll:true});field.value=${JSON.stringify(feedback)};field.dispatchEvent(new Event('input',{bubbles:true}));})()`);
    await click(control('[data-review-action="recheck"]'));
    await waitFor(`!document.querySelector(${JSON.stringify(control('[data-review-action="save-feedback"]'))})?.disabled`, 'Rechecked Assembly proposal');
    ...
    await evaluate(`window.__assemblyReviewEvidence={originalFetch:window.fetch,requests:[],drop:true};window.fetch=async function(input,options){const evidence=window.__assemblyReviewEvidence,url=String(input);if((options?.method??'GET').toUpperCase()==='POST'&&url.includes('/reviews/')&&url.endsWith('/feedback')){evidence.requests.push(String(options.body));const response=await evidence.originalFetch.call(this,input,options);if(evidence.drop&&response.ok){evidence.drop=false;await response.clone().text();throw new TypeError('Synthetic loss after successful Assembly review');}return response;}return evidence.originalFetch.call(this,input,options);};`);
    try {
      await click(control('[data-review-action="save-feedback"]'));
      await waitFor(`Boolean(document.querySelector(${JSON.stringify(control('[data-review-action="retry"]'))}))`, 'Unconfirmed Assembly decision');
      await click(control('[data-review-action="check"]'));
      await waitFor(`document.querySelector(${JSON.stringify(root)})?.textContent.includes('Retry the retained exact request')`, 'Saved outcome refresh');
      assert.equal(await evaluate(`Boolean(document.querySelector('[data-review-action="retry"]'))`),true);
      assert.equal(await evaluate(`document.querySelector(${JSON.stringify(control('[data-review-input="summary"]'))}).value`), feedback);
      await click(control('[data-review-action="retry"]'));
      await waitFor(`!document.querySelector(${JSON.stringify(control('[data-review-action="retry"]'))})`, 'Replayed Assembly review');
      ...
      const after = await project(), proposal = after.snapshot.reviewLibrary.groups.find(item => item.legacySource?.proposalId === proposalId);
      assert.equal(after.revision, before.revision + 1); assert.equal(proposal.status, 'CHANGES_REQUESTED'); assert.equal(proposal.feedback.summary, feedback);
      assert.deepEqual(after.snapshot.assemblyLibrary, before.snapshot.assemblyLibrary,'Legacy Assembly records remain immutable during shared adoption');
