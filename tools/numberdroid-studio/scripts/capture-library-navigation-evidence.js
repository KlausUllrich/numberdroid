...
export async function captureLibraryNavigation({ devtools, sessionId, captureCheckpoint = async () => {} }) {
  ...
  const review = '[data-review-workspace]', control = suffix => `${review} ${suffix}`;
  await waitFor(`Boolean(document.querySelector(${JSON.stringify(control('[data-review-canvas]'))}))`, 'Existing exact Assembly review');
  ...
  await evaluate(`(()=>{const n=document.querySelector(${JSON.stringify(control('[data-review-input="summary"]'))});n.value=${JSON.stringify(feedback)};n.dispatchEvent(new Event('input',{bubbles:true}));})()`);
  await click(control('[data-review-details="current"]'));
  ...
  assert.equal(await evaluate(`document.querySelector(${JSON.stringify(control('[data-review-focus="side:current"]'))})?.getAttribute('aria-pressed')`), 'true');
  assert.deepEqual(await project(), before);
  result.reviewDetailRetention = true; await captureCheckpoint('review-return');
  await click(control('[data-review-action="cancel-feedback"]'));
  await evaluate(`window.__libraryReviewAudit={fetch:window.fetch,requests:[],drop:true};window.fetch=async function(input,options){const a=window.__libraryReviewAudit,url=String(input);if((options?.method??'GET').toUpperCase()==='POST'&&url.includes('/reviews/')&&url.endsWith('/accept')){a.requests.push(String(options.body));const response=await a.fetch.call(this,input,options);if(a.drop&&response.ok){a.drop=false;await response.clone().text();throw new TypeError('Synthetic lost Library decision receipt');}return response;}return a.fetch.call(this,input,options);};`);
  try {
    await click(control('[data-review-action="accept"]'));
    await waitFor(`Boolean(document.querySelector('[data-review-action="retry"]'))`, 'Unconfirmed Library review');
    await evaluate("document.querySelector('[data-library-action=back]')?.click()"); await settle();
    assert.equal(await evaluate(`Boolean(document.querySelector('[data-review-action="retry"]'))`),true,'Back must preserve an unresolved decision');
    await click(control('[data-review-action="check"]'));
    await waitFor(`document.querySelector(${JSON.stringify(review)})?.textContent.includes('Retry the retained exact request')`, 'No inferred receipt after refresh');
    await click(control('[data-review-action="retry"]'));
    await waitFor(`document.querySelector('[data-review-status]')?.dataset.reviewStatus==='ACCEPTED' && !document.querySelector('[data-review-action="retry"]')`, 'Exact Library review retry');
    ...
    const accepted=after.snapshot.reviewLibrary.groups.find(group=>group.legacySource?.proposalId===proposalId); assert.equal(accepted.status,'ACCEPTED'); assert.equal(accepted.reviewVersion,1);
    assert.deepEqual(after.snapshot.assemblyLibrary.proposals,before.snapshot.assemblyLibrary.proposals,'Legacy history must remain unchanged');
    const savedAssembly=after.snapshot.assemblyLibrary.assets.find(asset=>asset.assetId===assemblyId), previousAssembly=before.snapshot.assemblyLibrary.assets.find(asset=>asset.assetId===assemblyId);
    const sourceProposal=before.snapshot.assemblyLibrary.proposals.find(proposal=>proposal.proposalId===proposalId);
    assert.equal(savedAssembly.assetVersion,previousAssembly.assetVersion+1);
    assert.deepEqual(savedAssembly.assembly,sourceProposal.content.assembly,'Acceptance saves the exact inspected proposed composition');
    ...
    const activity=await evaluate(`fetch('/api/projects/${projectId}/activity?presentation=true').then(r=>r.json())`);
    const acceptedEvent=activity.events.find(event=>event.presentation?.inspect?.reviewId===accepted.reviewId && event.presentation?.inspect?.reviewVersion===1);
    assert(acceptedEvent,'Accepted shared Review event must identify exact Review v1');
    const history=`[data-activity-inspect="${acceptedEvent.id}"]`;
    ...
    assert.equal(await evaluate(`document.querySelector('[data-review-workspace]')?.dataset.reviewId`),accepted.reviewId);
    assert.equal(await evaluate(`document.querySelector('[data-review-workspace]')?.dataset.reviewVersion`),'1');
    assert.equal(await evaluate(`document.querySelectorAll(${JSON.stringify(review + ' [data-review-action="accept"]:not(:disabled), ' + review + ' [data-review-action="request-changes"]:not(:disabled), ' + review + ' [data-review-action="discard"]:not(:disabled)')}).length`), 0);
