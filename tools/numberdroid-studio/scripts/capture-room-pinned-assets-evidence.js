...
export async function captureRoomPinnedAssets({ devtools, sessionId, width, height, pageUrl, outputPath, domPath, browserVersion }) {
  ...
  await evaluate(`(() => { const original = window.fetch.bind(window); window.__pinnedAudit = { posts: [], holdMixed: false, held: false, release: null }; window.fetch = async (input, options) => { const pathname = new URL(typeof input === 'string' ? input : input.url, location.href).pathname; if ((options?.method ?? 'GET').toUpperCase() === 'POST') window.__pinnedAudit.posts.push({ pathname, body: JSON.parse(options.body ?? '{}') }); const response = await original(input, options); if (window.__pinnedAudit.holdMixed && pathname.includes('/${MIXED_ROOM}/') && pathname.endsWith('/pinned-assets')) { window.__pinnedAudit.holdMixed = false; await response.clone().arrayBuffer(); return new Promise((release) => { window.__pinnedAudit.release = () => release(response); window.__pinnedAudit.held = true; }); } return response; }; })()`);
