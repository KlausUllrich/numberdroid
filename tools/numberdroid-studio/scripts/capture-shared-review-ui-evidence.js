...
export async function captureSharedReviewUi({ devtools, sessionId, captureCheckpoint = async () => {}, reviseProposal }) {
  ...
  assert(proposed.every(item => item.actions.length > 0 && item.actions.every(action => action === 'review')));
  assert(proposed.every(item => item.savedIdentity === false));
  await captureCheckpoint('proposed-additions');
  await click('[data-library-status=proposed][data-library-item-id=image] [data-library-action=review]'); await version(1);
  assert.equal(await evaluate("document.querySelector('[data-review-canvas]').dataset.reviewPreviewItem"), 'image');
  result.proposedAdditions = { count: proposed.length, exactItemNavigation: true, savedOrUseActions: false };
