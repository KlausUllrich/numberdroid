...
test('A1.7 render remains after Current step, before task facts, and contains no new action control', async () => {
  ...
  assert.match(taskDetail, /detail\.append\(workflow\);\s*if \(reviewHasConflict\) detail\.append\(renderTaskReview\(selected\)\);\s*if \(taskMayLoadProcessingAdoption\(selected\)\) detail\.append\(renderProcessingAdoption\(selected\)\);[\s\S]*?const factsSection/);
...
test('A1.7 passive refresh fingerprints bounded attempts and preserves compatible DOM context at protected widths', async () => {
  ...
  assert.match(app, /requestTaskAdoptionProjection\(projectId, selectedTaskId, \{ signal \}\)/);
