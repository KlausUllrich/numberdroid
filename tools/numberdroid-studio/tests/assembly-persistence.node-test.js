...
test('Migration 0016 rolls back before and after DDL, resumes, and preserves all earlier checksums', { timeout: 120000 }, async context => {
  ...
  const migrations=await loadMigrationDefinitions();assert.equal(migrations.at(-1).version,16);
  for(const point of ['before_migration_16','after_migration_16']) {
    ...
    try{assert.equal(resumed.integrityCheck().userVersion,16);assert.equal(resumed.workspace.database.prepare("SELECT strict FROM pragma_table_list WHERE name='assembly_versions'").get().strict,1);}
