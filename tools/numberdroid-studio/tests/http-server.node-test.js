...
test('visual shell is clickable, creates the demo through commands, and exposes live activity', async (context) => {
  ...
  assert.match(clientScript, /rectangle\[key\] !== drag\.original\[key\][\s\S]*!drag\.changed[\s\S]*drag\.changed = true; markCutterDefinitionDirty\(\)/);
  assert.match(clientScript, /!drag\.svg\.isConnected \|\| !drag\.target\.isConnected \|\| drag\.instanceId !== state\.cutter\.instanceId/);
  ...
  const cutterViewResponse = await fetch(`${base}/cutter-editor-view.js`);
  assert.equal(cutterViewResponse.status, 200);
  const cutterViewScript = await cutterViewResponse.text();
  assert.match(clientScript, /const cutter = state\.cutter[\s\S]*renderCutterEditor\(\{ cutter, source, atlas/);
  assert.match(cutterViewScript, /scroll\.dataset\.cutterScrollContext =/);
  assert.match(cutterViewScript, /input\('range'/);
  assert.match(cutterViewScript, /action\('100%', \{ cutterZoomAction: '1' \}\)/);
  assert.match(cutterViewScript, /cutter\.frozenScale \?\?/);
  assert.match(cutterViewScript, /\['nw', 'n', 'ne', 'e', 'se', 's', 'sw', 'w'\]/);
  assert.match(cutterViewScript, /rectangleField: 'name'/);
  assert.match(cutterViewScript, /rectangleField: 'included'/);
  assert.match(cutterViewScript, /cutterSelect: String\(index\)/);
  assert.match(cutterViewScript, /cutter-tool-rail/);
  assert.match(cutterViewScript, /cutter-grid-popup/);
  assert.match(cutterViewScript, /status\.setAttribute\('role', 'status'\)/);
  assert.match(cutterViewScript, /Snap to grid · hold Alt to bypass/);
  assert.match(cutterViewScript, /Create cuts from grid/);
  assert.match(cutterViewScript, /cutterHistory|cutter\.history\.past\.length/);
  ...
  const cutterEvidenceScript = await readFile(
    new URL('../scripts/capture-cutter-editor-evidence.js', import.meta.url), 'utf8',
  );
  assert.match(browserEvidenceScript, /import \{ inspectCutterEditor, captureCutterEditor \} from '\.\/capture-cutter-editor-evidence\.js'/);
  assert.match(browserEvidenceScript, /layout\.cutter = await inspectCutterEditor\(\{ devtools, sessionId, focus: checkpoint2bFocus \}\)/);
  assert.match(browserEvidenceScript, /checkpoint2bInteractionEvidence = await captureCutterEditor\(\{ devtools, sessionId \}\)/);
  assert.match(cutterEvidenceScript, /scroll\.scrollLeft=321; scroll\.scrollTop=417/);
  assert.match(cutterEvidenceScript, /await refresh\(\); await refresh\(\)/);
  assert.match(cutterEvidenceScript, /assert\(passive\.sameScroller && passive\.sameCanvas && passive\.sameField && passive\.focused\)/);
  assert.match(cutterEvidenceScript, /assert\.equal\(passive\.value, 'Refresh draft name'\)/);
  assert.match(cutterEvidenceScript, /assert\.deepEqual\(passive\.scroll, passive\.before\)/);
  assert.match(cutterEvidenceScript, /assert\.deepEqual\(passive\.page, passive\.beforePage\)/);
  assert.match(cutterEvidenceScript, /assert\(passive\.scroll\.every\(n=>n>0\)\)/);
  assert.match(cutterEvidenceScript, /assert\(!projection\.scrollerReplaced && projection\.fieldReplaced && projection\.focused/);
  assert.match(cutterEvidenceScript, /assert\.equal\(projection\.value,passive\.value\)/);
  assert.match(cutterEvidenceScript, /assert\.deepEqual\(projection\.scroll,passive\.scroll\)/);
  assert.match(cutterEvidenceScript, /assert\.deepEqual\(projection\.page,passive\.page\)/);
  assert.match(cutterEvidenceScript, /evidence\.scrollPreservation=passive/);
  assert.match(cutterEvidenceScript, /evidence\.necessaryRerender=projection/);
  const nativeDragStart = cutterEvidenceScript.indexOf('const nativeDrag = async');
  const nativeDragEnd = cutterEvidenceScript.indexOf('await evaluate(`(() => { window.__cutterEvidence={', nativeDragStart);
  const nativeDrag = cutterEvidenceScript.slice(nativeDragStart, nativeDragEnd);
  const press = nativeDrag.indexOf("await pointer('mousePressed'");
  const move = nativeDrag.indexOf("await pointer('mouseMoved'");
  const forcedRender = nativeDrag.indexOf('forceChangedCutterProjectionRender()');
  const release = nativeDrag.indexOf("await pointer('mouseReleased'");
  assert.ok(press >= 0 && move > press && forcedRender > move && release > forcedRender,
    'Native pointer press/move must precede the forced projection, with release in final cleanup.');
  assert.match(cutterEvidenceScript, /devtools\.send\('Input\.dispatchMouseEvent'/);
  assert.match(cutterEvidenceScript, /button: 'left'/);
  assert.match(cutterEvidenceScript, /buttons: type === 'mouseReleased' \? 0 : 1/);
  assert.match(nativeDrag, /assert\.equal\(pressed\.interaction\.hasPointerCapture, true\)/);
  assert.match(nativeDrag, /window\.__cutterDragProbeTarget\s*=\s*window\.__cutterEvidence\.target/);
  assert.match(nativeDrag, /window\.__cutterDragProbeTarget\s*=\s*null/);

  assert.match(nativeDrag, /assert\.equal\(moved\.interaction\.dragActive, true/);
  assert.match(nativeDrag, /assert\.equal\(moved\.interaction\.pointerId, pressed\.interaction\.pointerId\)/);
  assert.match(nativeDrag, /e\.type === 'pointerdown' && e\.pointerId === pressed\.interaction\.pointerId && e\.buttons === 1 && e\.exactProbeTarget/);
  assert.match(nativeDrag, /e\.type === 'pointermove' && e\.pointerId === pressed\.interaction\.pointerId && e\.buttons === 1 && e\.exactProbeTarget/);
  assert.doesNotMatch(nativeDrag, /assert\.equal\(moved\.interaction\.hasPointerCapture, true\)/);
  assert.match(nativeDrag, /assert\(!settled\.replaced && !settled\.oldDisconnected/);
  assert.match(nativeDrag, /assert\.deepEqual\(settled\.inspector, after\.geometry\[0\]\)/);
  assert.match(nativeDrag, /assert\.deepEqual\(repeated\.canvas, before\.canvas/);
  assert.match(nativeDrag, /assert\.deepEqual\(repeated\.geometry, position === start \? before\.geometry : moved\.geometry/);

  assert.match(nativeDrag, /assert\.equal\(moved\.interaction\.changed, true\)/);
  assert.match(nativeDrag, /assert\.equal\(moved\.interaction\.dirty, true\)/);
  assert.match(nativeDrag, /assert\.deepEqual\(moved\.canvas, before\.canvas/);
  assert.match(nativeDrag, /assert\.equal\(during\.interaction\.deferred, true\)/);
  assert.match(nativeDrag, /assert\.equal\(during\.interaction\.pointerId, pressed\.interaction\.pointerId\)/);
  assert.match(nativeDrag, /assert\.deepEqual\(during\.scroll, before\.scroll\)/);
  assert.match(nativeDrag, /assert\.deepEqual\(during\.page, before\.page\)/);
  assert.match(nativeDrag, /finally \{ await pointer\('mouseReleased'/);
  assert.match(nativeDrag, /assert\.equal\(after\.interaction\.deferred, false\)/);
  assert.match(nativeDrag, /assert\.equal\(after\.interaction\.hasPointerCapture, false\)/);
  assert.match(nativeDrag, /assert\.deepEqual\(cleanup, \{ released: true, targetReleased: true, traceCleared: true \}\)/);
  assert.ok(nativeDrag.indexOf('resetCutterPointerTrace()') < press
    && nativeDrag.indexOf('clearCutterPointerTrace()') > release,
    'Pointer telemetry resets before the gesture and clears only after release.');
  assert.match(cutterEvidenceScript, /evidence\.dragContinuity=await nativeDrag\('\[data-cutter-move="0"\]'/);
  assert.match(cutterEvidenceScript, /assert\.deepEqual\(evidence\.dragContinuity\.after\.geometry\[0\],\[20,14,622,622\]\)/);
  assert.match(cutterEvidenceScript, /assert\.deepEqual\(reopened\.geometry,CANONICAL_RECTS\)/);
  assert.match(cutterEvidenceScript, /assert\.equal\(reopened\.zoom,'fit'\)/);
