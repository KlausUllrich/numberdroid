...
test('Shared Review official MCP negotiates27/8, revises feedback and rechecks HostBinding before query/replay', { timeout: 120000 }, async t => {
  ...
  const unnegotiatedCommand = await f.request('review.proposal.submit', group(f));
  const beforeHandshake = await f.studio.readProjectTrusted(projectId);
  const rawSubmit = await fetch(`${base}/internal/mcp/execute`, { method: 'POST', headers: { authorization: `Bearer ${issued.token}`, 'content-type': 'application/json' }, body: JSON.stringify({ schemaVersion: 1, command: unnegotiatedCommand }) });
  ...
  await legacyGateway.negotiateReviewV1({ schemaVersion: 1, projectId, profile: 'review-v1' });
  const missingScopeCommand = await f.request('review.proposal.submit', group(f));
  const beforeMissingScope = await f.studio.readProjectTrusted(projectId);
  await assert.rejects(legacyGateway.execute(missingScopeCommand), { code: 'GRANT_SCOPE_MISSING' });
  assert.deepEqual(await f.studio.readProjectTrusted(projectId), beforeMissingScope);
  ...
  try {
    ...
    const rawBinding = hostBindingStore.issue({ projectId, grantId: 'grant.review', agentId: 'agent.review', taskId: 'task.review', branchId: 'branch.main', issuedBy: owner.actor.id });
    const rawRead = () => fetch(`${base}/internal/mcp/read-project`, { method: 'POST', headers: { authorization: `Bearer ${rawBinding.token}`, 'content-type': 'application/json' }, body: JSON.stringify({ schemaVersion: 1, projectId }) });
    assert.equal((await (await rawRead()).json()).error.code, 'REVIEW_NEGOTIATION_REQUIRED');
    const rawGateway = new LocalStudioGateway({ baseUrl: base, bindingToken: rawBinding.token });
    await rawGateway.negotiateReviewV1({ schemaVersion: 1, projectId, profile: 'review-v1' });
    assert.equal((await (await rawRead()).json()).snapshot.reviewLibrary.groups[0].reviewId, reviewId);

    const beforeChangedReplay = await f.studio.readProjectTrusted(projectId);
    await assert.rejects(submitTool.execute({ ...command, payload: { ...command.payload, title: 'Changed replay must fail' } }), { code: 'IDEMPOTENCY_CONFLICT' });
    assert.deepEqual(await f.studio.readProjectTrusted(projectId), beforeChangedReplay);
    const ownerDecision = await f.request('review.accept', { reviewId, expectedReviewVersion: 1, selectedItemIds: ['item.image'], confirmed: true });
    await assert.rejects(gateway.execute(ownerDecision), { code: 'FORBIDDEN' });
    assert.deepEqual(await f.studio.readProjectTrusted(projectId), beforeChangedReplay);
    const before = await f.studio.readProjectTrusted(projectId);
    ...
    await assert.rejects(submitTool.execute(command), { code: 'REVIEW_TASK_BRANCH_UNSUPPORTED' });
